...
**发布的客户端必须完全不依赖运行环境**：用户机器上不需要预装 Node、Python、浏览器、解码器或任何运行时；产物完全离线可用，不访问网络；同一版本 + 同一插件集行为一致。细则与豁免见 [docs/environment-independence.md](docs/environment-independence.md)（唯一 home）。发布门禁机械校验此约束；任何违反它的改动（CDN、外置运行时、运行时下载、环境变量分支）直接拒绝，除非先在该文档登记豁免并说明理由。
...
pnpm verify:dep-audit    # 依赖白名单 diff + node: 引用扫描
...
- 本地文件访问只走 `src-tauri/src/lib.rs` 的命令 + asset protocol，扩展名分派在 Rust 侧完成。
