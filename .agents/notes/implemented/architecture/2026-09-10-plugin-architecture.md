...
- **窗口销毁权限补齐**（2026-09-13 PR #15 检视 §6.14 修复）：实测点 X 关窗零响应——关闭守卫注册 `onCloseRequested` 后 tauri 无条件 `prevent_close()`，关窗全押在 wrapper 自动 `destroy()` 上，而 capabilities 只授只读集合，`plugin:window|destroy` 被 ACL 拒、拒绝落在包装层 promise，release 下无可见出口；自本 Note 的 guardClose 落地即坏（验收只测过脏态"取消"路径，vitest mock 掉 Tauri 边界、权限面无测试射程）。修法：capabilities 增 `core:window:allow-destroy` + vitest 钉子读配置断言权限在位（权限面失败无运行期症状可测，只能钉配置面）。
