...
- **native-links darwin raw-binary fallback** (fix for PR #15 review finding §6.13, 2026-09-13): measured — tauri's dmg bundling step "Cleaning"s away the `.app` under `bundle/macos/`, so on macOS running `pnpm tauri build` → `verify:release` in order left native-links with nothing to scan and always red; the `.app`'s main binary is byte-identical to the `target/release` raw product, so the collector now falls back to scanning the raw binary when no `.app` is present — equivalent link audit, and the release gate is immune to dmg cleanup whatever the order (the linux branch already scanned the raw binary; the two platforms are now unified); the failure message is now platform-neutral.
- **Window destroy permission backfilled** (fix for PR #15 review finding §6.14, 2026-09-13): measured — clicking X never closed any window. Once the close guard registers an `onCloseRequested` listener, tauri unconditionally `prevent_close()`s the native close and closing rides entirely on the API wrapper's automatic `destroy()`, which the ACL denied: capabilities granted only the read-only window set, and the rejection lands inside the wrapper's promise chain with no visible outlet in release. Broken since this Note's guardClose landed (acceptance only covered the dirty-state "cancel" path; vitest mocks the Tauri boundary, so the permission surface had no test range). Fix: add `core:window:allow-destroy` to capabilities plus a vitest pin reading the config file asserting the permission is present (permission failures have no runtime symptom — only the config side can be pinned).
