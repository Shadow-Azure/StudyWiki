...
- **Resizable sidebar**: a full-height 1px hairline sits between sidebar and main; the line occupies no grid track, with only an invisible 7px pointer target behind it. Dragging writes `--sidebar-size`, while 210–380px bounds keep the 720px minimum window free of toolbar/content overflow. Arrow/Home/End keys update `aria-valuenow`.
