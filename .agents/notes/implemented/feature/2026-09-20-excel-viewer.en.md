...
- Raw binary IPC gains in-process integration tests: both read and write commands traverse Tauri MockRuntime's real IPC resolver (`InvokeRequest` + raw body/path header). macOS requires WebviewWindow creation on the main thread, so that test crate uses `harness = false`. On Windows CI the MockRuntime integration executable consistently fails to load with `STATUS_ENTRYPOINT_NOT_FOUND`; Windows therefore runs an explicit skip driver while macOS/Linux CI keep the real-resolver cases.
