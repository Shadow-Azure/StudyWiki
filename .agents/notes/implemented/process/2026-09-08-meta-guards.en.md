...
- Added the postmortem structure gate `verify-postmortem` (initially deferred until the first incident file; the user chose to establish it now): NNNN-topic numbering contiguous and unique from 0001, the four headings (Executive summary / Timeline / Root cause / Action items) verbatim and in order on both sides, `SECTIONS` mirrored against the README (asserted by spec); wired into doc-quick and doc-sync.
...
- Left open: the site projection step is absent pending a product decision (analysis in [2026-09-08-close-pipeline-gaps.en.md](2026-09-08-close-pipeline-gaps.en.md)).
