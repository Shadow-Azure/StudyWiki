...
- `pnpm verify:dep-audit` ([scripts/verify-dep-audit.mjs](../scripts/verify-dep-audit.mjs)): a diff against the `dependencies` allowlist — any unregistered new package goes red; the transitive closure is scanned for `node:` builtin references.
- `pnpm verify:layering` ([scripts/verify-layering.mjs](../scripts/verify-layering.mjs)): `src/plugins/` reaching `@tauri-apps/*` or host implementations goes red (type-only imports pass); dynamic `import()` / `eval` / `new Function` under `src/` is allowed only through the single allowlisted seam (`src/loader/external.ts`, the external-plugin blob loading channel).
...
- The linux-side ureq/rustls link scan: a pure-Rust stack should add zero new dylibs in theory (the native-links allowlist covers the system C runtime); pending verification on the first real linux release.
