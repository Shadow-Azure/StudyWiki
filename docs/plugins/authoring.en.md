...
Start the host with `pnpm tauri dev` → plugin panel → “本地导入…” (import local) → pick the tgz produced by `npm pack` → it takes effect in this window immediately (directory and manifest land together; other windows follow at their next start). The full hot-path behaviour contract lives in [dynamic.en.md](dynamic.en.md); broken rows are named in the panel for cleanup and don't affect other plugins.
