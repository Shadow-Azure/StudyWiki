...
外置插件是零依赖单文件 ESM 模块，以 npm tarball 形态进入宿主，下列要求全部有机械校验兜底——宿主/生成器/发布校验的分工见表内标注。
...
`pnpm tauri dev` 起宿主 → 插件面板 → “本地导入…” 选 `npm pack` 产出的 tgz → 本窗立即生效（目录与清单同时落盘，其他窗口下次启动跟随）。**tgz 只用 `npm pack` 产出**：系统 `tar`（macOS 带扩展属性）写出的 `._*` 条目会被拒收。热路径的完整行为契约见 [dynamic.md](dynamic.md)；装坏的行面板点名待清理，不影响其余插件。
