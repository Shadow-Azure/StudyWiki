...
- 段落一行到底（soft-wrap），表格与代码块保持原样。
- 文档内代码示例必须可用；贴类型声明用 ` ```ts type-equiv ` 围栏（源码漂移即红）；命令面走生成区（`pnpm gen:commands`），标记之间不手编。
...
```sh
pnpm lint:docs    # doc-quick：索引、Note、预算、配对、type-equiv（秒级）
