...
- 文档内代码示例必须可用；贴类型声明用 ` ```ts type-equiv ` 围栏（源码漂移即红）；命令面与架构组成树走生成区（`pnpm gen:commands` / `pnpm gen:code-map`），标记之间不手编。导出面须带文档注释（verify-export-docs）。
...
```sh
pnpm lint:docs    # doc-quick：索引、Note、预算、段落换行、配对、type-equiv、导出注释、源码引用、组成树、postmortem 结构（秒级）
pnpm verify:docs  # doc-sync：上述 + ts 围栏编译 + 生成区新鲜度 + 死链 + 环境无关 + 门禁自测试
```
