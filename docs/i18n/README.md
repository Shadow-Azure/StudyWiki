...
两侧结构必须镜像：标题层级与顺序、代码围栏（info 串与内容**逐字相同**——围栏内的注释、图示文字不翻译）、表格行列数、列表种类/有序起点/条数、链接目标（切换行除外）。语料内相对链接：base 侧用 `.md`、英文侧用 `.en.md`；query/fragment 逐字保留。生成区（`<!-- BEGIN GENERATED -->`）除 locale 投影的文档路径外逐字节一致。
